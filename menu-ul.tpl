		...
		<ul class="{$class}{if $pid} menu_{$pid}{/if}">
			...
			{foreach $data.$pid as $menu}
				...
				{else}
					<li class="m_{$menu.page_name}
						...
						>
							{$menu.text}
							{if (isset($data[$menu.el_id]) || isset($menu_children)) && $menu.level == 0  && $position != 'left'}<span class="fa fa-angle-down text-primary"></span>{/if}
						</a>
			...
			{if isset($dropdown) && $menu.level < 1}
				<li class="dropdown dropdown-more">
					<a class="dropdown-toggle" data-toggle="dropdown" href="#">{lang key='more'} <span class="fa fa-angle-down text-primary"></span></a>
